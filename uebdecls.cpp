...
//__host__ __device__ 
void uebCell::setConstantValues()
{
...
//__host__ __device__ 
void uebCell::setParams(float Params[32])
{
...
//__host__ __device__ 
void uebCell::setSiteVars_and_Initconds(float SiteVars[32])
{
	for (int i = 0; i < 32; i++)
...
//__host__ __device__ 
void uebCell::setModelRun_Settings(int startDate[3], int endDate[3], double  startHour, double  endHour, double  modeldT, double  UTCoffset, int inpDailyorSubd, int oStride)
{
	for (int i = 0; i<3; i++)
	{
...
//5.2.15 from snowdv
	...
	//for (int i = 0; i < 70; i++)
	//OutVarValues = new float[70*numTotalTimeSteps];         // *outtStride];
...
//7.22.16
/*__host__ __device__*/ void uebCell::setInitialEnsembleStates(int nEns)   //, const char* forcName)
{
	for (int ie = 0;ie< nEns + 1; ie++)            //nEns + 1 the extra entry for ensemble mean
	{	
		for (int is = 0; is < 6; is++)
			stateVS[is].push_back(statev[is]);
//#*$8.8.16 7th state for snow surface temp 
		stateVS[6].push_back(tsprevday[nstepinaDay-1]);
		//8th for snow ave. temp Tave
		stateVS[7].push_back(taveprevday[nstepinaDay - 1]);
		
		cumPV.push_back(cumP);
		cumEsV.push_back(cumEs);
		cumEcV.push_back(cumEc);                // Evaporation from canopy
		cumMrV.push_back(cumMr);                 // canopy melt not added
		cumGmV.push_back(cumGm);             //  Cumulative glacier melt
		cumEgV.push_back(cumEg);
		dStorageV.push_back(dStorage);
		errMBV.push_back(errMB);
	}
...
void uebCell::getInpForcArr(int numNc[13], float** RegArray[13], float &tcorVar, MPI::Intracomm inpComm, MPI::Info inpInfo)
{
	for (int it = 0; it < 13; it++)
	{
		...
		{
			//tEnd = ncReadStart[it] + 1;          //read only one time step 7.20.16 instead of 24 previously 
			//offSet = 1;    // uebCellY*dimLen2*numTotalTimeSteps + uebCellX*numTotalTimeSteps;
			int retvalue = 0;			
...
void uebCell::setInpForcArr(int it, float **inArray, float &forcArr)  //, int ncTotaltimestepit)
{
	...
	else if (infrContArr[it].infType == 2 || infrContArr[it].infType == -1)
	{
		// use default value or compute internally		
			...
			forcArr = inArray[uebCellY][uebCellX];
		
}
// read input text file and record datetime, value pair --skip no data, get no data value from file
void  uebCell::readTStextFileTimeValPair(const char* inforcFile, std::vector<std::pair<double, float>> &tvar_in, int &nrecords)
{
