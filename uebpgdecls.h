...
//
//
#ifndef __UEBPGDECLS_H
#define __UEBPGDECLS_H
...
class uebCell {  
	...
	public:
	   ...
	   //read parameter values
		...
		int getforcOffset(int ioffst, int dimLen2);
		 ...
		 /*__host__ __device__*/ 
		 void runUEBEnsembles(int nEns, float** ensForcingMultiplier, float** &stateOutput);                   // , float* &ensAnomaly, float &ensMean);  // , bool NormalDist);
		 //save only state with observation
		 void runUEBEnsembles(int nEns, float** ensForcingMultiplier, int outStateIndex, float* &stateOutput);
		 /*__host__ __device__*/ 
		 void updateBackgroundStates(int nEns, float** updateStateArr);
		 /*__host__ __device__*/ 
		 void runUEB();
		 /*__host__ __device__*/
		 ...
		 void getInpForcArr(int numNc[13], float** RegArray[13], float &tcorVar, MPI::Intracomm inpComm, MPI::Info inpInfo);
		...
		//outputs
		float OutVarValues[70];                     //4.28.15 compute max of 96 time steps at time (for daily input, 4 days with dT = 6h)
		...
		float  PrecArr, TempArr, TaminArr, TamaxArr, WindspArr, RhArr,
			VpArr, ApresArr, SradArr, LradArr, NradArr, QgArr, SnowalbArr;
    ...
    private:
		std::vector<float> stateVS[8];      // 0, stateV1, stateV2, stateV3, stateV4, stateV5;    //8.8.16 7th state for snow surface temp, 8th Tave
